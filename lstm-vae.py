...
all_df.rename({'anomaly': 'real_anomaly'}, axis=1, inplace=True)
...
class Encoder(layers.Layer):
    def __init__(self, time_step, x_dim, lstm_h_dim, z_dim, name='encoder', **kwargs):
        ...
        self.z_mean = layers.Dense(z_dim, name='z_mean', activation='tanh')
        self.z_logvar = layers.Dense(z_dim, name='z_log_var', activation='tanh')
...
class Decoder(layers.Layer):
    def __init__(self, time_step, x_dim, lstm_h_dim, z_dim, name='decoder', **kwargs):
        ...
        self.x_mean = layers.Dense(x_dim, name='x_mean', activation='tanh')
...
def plot_log_likelihood(df_log_px):
    ...
    sns.set_color_codes()
    sns.distplot(df_log_px, bins=40, kde=True, color='blue')
...
df = all_df.drop(["real_anomaly","changepoint"], axis=1)

train_scaled, test_scaled = split_normalize_data(df[:37000])
...
#trainX = to_sequences(train_scaled, timesteps)
#testX = to_sequences(test_scaled, timesteps)

train_X = reshape(train_scaled)
test_X = reshape(test_scaled)

opt = keras.optimizers.Adam(learning_rate=0.001, epsilon=1e-2, amsgrad=True)
...
_, _, train_log_px = model.predict(train_X, batch_size=10)
...
x = train_log_px
y = np.array(all_df[:37000][:27750]["real_anomaly"]).reshape(-1,1)
...
_, _, test_log_px = model.predict(test_X, batch_size=10)
...
#anomaly = {-1:1, 1:0}
y_pred =knn.predict(test_log_px)
#y_pred = list(pd.Series(y_pred).map(anomaly))
test_df = all_df[:37000][27750:]

df_log_px["pred"] = y_pred
test_df["pred_anomaly"] = list(df_log_px['pred'])


print(confusion_matrix(test_df["real_anomaly"], test_df["pred_anomaly"]))
print(f1_score(test_df["real_anomaly"], test_df["pred_anomaly"]))
print(accuracy_score(test_df["real_anomaly"], test_df["pred_anomaly"]))
print(roc_auc_score(test_df["real_anomaly"], test_df["pred_anomaly"]))